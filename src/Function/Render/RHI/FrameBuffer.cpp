...
FrameBuffer::FrameBuffer(uint32_t width, uint32_t height, AttachmentFormat type)
    ...
    if (attachStatus != GL_FRAMEBUFFER_COMPLETE)
        std::cout << "ERROR::FRAMEBUFFER:: Framebuffer is not complete!" << attachStatus << std::endl;

    // Unbind(); 
    GLCall(glBindFramebuffer(GL_FRAMEBUFFER, 0));
}

FrameBuffer::FrameBuffer(uint32_t width, uint32_t height, AttachmentFormat type, std::int32_t numbers , bool depth)
        : m_Width(width), m_Height(height), m_Type(type), numAttachments(numbers), usingDepth(depth) {
