...
private:
    ...
    void CreateMassNodeByVertex(TexturedVertex_new &vertex);

    void InitSpringStructures(int rowIndex, int ColumnIndex);


    static inline glm::vec3 ComputeTriangleNormal(TexturedVertex_new &n1, TexturedVertex_new &n2,
                                           TexturedVertex_new &n3) {
...
public:
    ...
    Cloth(glm::vec3 position, std::vector<TexturedVertex_new> &clothObj);

    std::vector<TexturedVertex_new> &GetTriangles();
    std::vector<TexturedVertex_new> GetNodes();
