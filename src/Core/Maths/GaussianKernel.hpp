...
namespace Maths
{
    ...
    /*constexpr */double gaussian2D(double x, double y, double sigma, double meanX, double meanY) {
        // Compute the exponent part of the Gaussian formula
        double exponent = -0.5 * (Pow2((x - meanX) / sigma) + Pow2((y - meanY) / sigma));
    ...
    constexpr std::array<double, KERNEL_SIZE* KERNEL_SIZE> GaussianKernel(double sigma = 1.0)
    {
        ...
        for (size_t i = 0; i < KERNEL_SIZE; i++) {
            for (size_t j = 0; j < KERNEL_SIZE; j++) {
                ...
                kernel[i * KERNEL_SIZE + j] = gaussian2D(i, j, sigma, mean, mean);
