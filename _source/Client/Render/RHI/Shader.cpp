...
namespace Sparrow
{
    ...
    ************************************************/
    ...
    int Shader::GetUniformLocation(const std::string& name) const
    {
        ...
        if (location == -1)
        {
            // LOG_WARN(LogModule::Asset, "Shader: {}, {} \n \t Uniform {} doesn't used!", vertPath.string(), fragPath.string(), name);
        }
    ...
    {
        // Get Shader Strings
        std::string vertCode = ParseShader(VERTEX, this->vertPath);
        std::string fragCode = ParseShader(FRAGMENT, this->fragPath);
        std::string tescCode = ParseShader(Tesc, this->tescPath);
        std::string teseCode = ParseShader(Tese, this->tesePath);

        CreateShaderProgram(vertCode, fragCode, tescCode, teseCode);
    }

    void Shader::CheckShaderCompileErrors(unsigned int shaderId, ShaderType shaderType)
    {
        int success;
        char infoLog[1024];
        if (shaderType != NONE)
        {
            ...
            if (!success)
            {
                ...
                LOG_ERROR(LogModule::Asset, "SHADER_COMPILATION_ERROR of type: {} \n {} \n ", (UInt)shaderType, infoLog);
            }
        }
    }
    ...
    unsigned int Shader::ReadAndCompileShader(ShaderType shaderType, const std::string& source)
    {
