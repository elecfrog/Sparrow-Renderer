...
#include "Client/Component/TransformComponent.h"
#include "Math/GaussianKernel.hpp"
...
namespace Sparrow
{
    ...
    public:
        // Texture | Slot
        HashMap<UInt, UInt> textureIdSlotMap;

        int counter = 0;

        void Include(UInt id, UInt slot)
        {
    ...
    class Scene_LoadModel : public Scene
    {
        ...
        static inline Matrix4f captureProjection = glm::perspective(glm::radians(90.0f), 1.0f, 0.1f, 10.0f);
        static inline Matrix4f captureViews[] =
        {
    ...
    public:
        Scene_LoadModel()
        {
            ...
            auto w_height = g_Engine.m_WindowSystem->GetWindowHeight();
            ...
            skybox_Shader = std::make_shared<Shader>(
                ShaderPath("skybox/cubemap.vert"), ShaderPath("skybox/cubemap.frag"));
            skybox_Shader->Bind();
        ...
        void OnRender() override
        {
            ...
            if (reloadShaders)
            {
                ...
                reloadShaders = false;
                LOG_INFO(LogModule::Scene, "Reload Shaders");
            }
             ...
             * */
            {
                ...
                for (auto&& mesh : scene_Model->GetMeshes())
                {
                    ...
                    MrtShader->SetUniform1u("material.emissiveTexture",
                ...
                for (int horizontal = 0; horizontal <= 1; ++horizontal)
                {
                    if (horizontal)
                    {
                        BlurShader->Bind();
                        {
                            ...
                            BlurShader->SetUniform1i("uBlurSourceImage",
                                                     textureManager.GetSlot(mrtFBO->GetTextureId(1)));
            ...
            mrtFBO->BindRead();
            mrtFBO->BindDraw();
            int window_width = g_Engine.m_WindowSystem->GetWindowWidth();
            int window_height = g_Engine.m_WindowSystem->GetWindowHeight();
            glBlitFramebuffer(
        ...
        void RegisterInputs() override
        {
            ...
            g_Engine.m_WindowSystem->RegisterOnFramebufferFunc(
