...
namespace Maths {
    ...
    inline glm::vec2
    WorldToScreen(const glm::vec3 &worldPos, const glm::mat4 &mvp, float width, float height, float winPosX = 0.0f,
                  float winPosY = 0.0f) {
    ...
    static glm::vec3 ComputeOrthogonalVector(const glm::vec3& vec) {
        // a random vector
        glm::vec3 arbitraryVec = glm::vec3(1.0f, 0.0f, 0.0f);
        
        if (glm::abs(glm::dot(vec, arbitraryVec)) > 0.98f) { // Check if they are nearly parallel
            arbitraryVec = glm::vec3(0.0f, 1.0f, 0.0f); // Choose another arbitrary vector
        }
        return glm::normalize(glm::cross(vec, arbitraryVec));
    }
}
