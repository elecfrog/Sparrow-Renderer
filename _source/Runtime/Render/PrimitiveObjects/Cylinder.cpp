...
namespace Sparrow
{
    Cylinder::Cylinder(Vector3 top_center, Vector3 bottom_center)
        : m_TopCenter(top_center), m_BottomCenter(bottom_center)
    {
        BuildMeshComponent();
    ...
    void Cylinder::BuildMeshComponent()
    {
        ...
        // 设置中心顶点
        component.m_PositionBuffer[4 * N + 0] = m_TopCenter; // 顶面中心
        component.m_PositionBuffer[4 * N + 1] = m_BottomCenter; // 底面中心

        // 计算法线
        Vector3 top_normal = (m_TopCenter - m_BottomCenter).Normalized();
        Vector3 bottom_normal = -top_normal;
        component.m_NormalsBuffer[4 * N + 0] = top_normal; // 顶面中心法线
        component.m_NormalsBuffer[4 * N + 1] = bottom_normal; // 底面中心法线

        // 计算圆柱体的基向量
        Vector3 normal = (m_BottomCenter - m_TopCenter).Normalized();
        Vector3 tangent = Maths::ComputeOrthogonalVector(normal);
        ...
        for (UInt i = 0; i < N; i++)
        {
            ...
            Float nx = cos(angle);
            Float ny = sin(angle);
            Vector3 offset = nx * tangent + ny * biTangent;

            // 设置顶点位置
            component.m_PositionBuffer[2 * i + 0] = m_TopCenter + R * offset; // 顶面边缘
            component.m_PositionBuffer[2 * i + 1] = m_BottomCenter + R * offset; // 底面边缘
            component.m_PositionBuffer[2 * N + i] = m_TopCenter + R * offset; // 顶面边缘（重复）
            component.m_PositionBuffer[3 * N + i] = m_BottomCenter + R * offset; // 底面边缘（重复）
            ...
            component.m_TangentsBuffer[2 * i + 0] = Vector3(0.0f);
            component.m_TangentsBuffer[2 * i + 1] = Vector3(0.0f);
            component.m_TangentsBuffer[2 * N + i] = Vector3(0.0f);
            component.m_TangentsBuffer[3 * N + i] = Vector3(0.0f);

            // 设置纹理坐标
            Float u = static_cast<Float>(i) / N;
            component.m_TexCoord0Buffer[2 * i + 0] = Vector2(u, 0.0f);
            component.m_TexCoord0Buffer[2 * i + 1] = Vector2(u, 1.0f);
            component.m_TexCoord0Buffer[2 * N + i] = Vector2(u, 0.0f);
            component.m_TexCoord0Buffer[3 * N + i] = Vector2(u, 1.0f);
        ...
        // 设置中心顶点的切线
        component.m_TangentsBuffer[4 * N + 0] = Vector3(0.0f);
        component.m_TangentsBuffer[4 * N + 1] = Vector3(0.0f);

        // 设置中心顶点的纹理坐标
        component.m_TexCoord0Buffer[4 * N + 0] = Vector2(0.5f, 0.5f);
        component.m_TexCoord0Buffer[4 * N + 1] = Vector2(0.5f, 0.5f);
    ...
    void Cylinder::BuildMeshRendererComponent()
    {
