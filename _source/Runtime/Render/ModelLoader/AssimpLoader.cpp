...
namespace Sparrow
{
    ...
    std::shared_ptr<Mesh> ProcessSkinnedMeshNode(Model* model, aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumBones; i++)
        {
            ...
            for (unsigned int j = 0; j < bone->mNumWeights; j++)
            {
                ...
                {
                    skinnedVertices[vertexID].boneIDs[slot] = (float)model->m_BoneInfo->nodes[bone->mName.C_Str()]->
                        boneID;
    ...
    [[nodiscard]] std::shared_ptr<Animation> ProcessAnimationNode(aiAnimation* animation, const aiScene* scene)
    {
        std::shared_ptr<Animation> tmp = std::make_shared<Animation>();
